...
function RaceGalleryCard({ gallery }) {
  if (!gallery?.enabled) return null;

  const galleries = gallery.galleries || [];
    ...
    >
      <div className="mb-4">
        <h2 className="text-sm font-bold text-sky-100 sm:text-base">
          {gallery.title || "Race Weekend Photo Gallery"}
        </h2>
...
function RaceWeekendRecapCard({ recap }) {
...
function MobileSessionRecapCard({ session, recap }) {
  if (!recap?.enabled || !session) return null;
  ...
  const recapSection = recap.sections.find((section) => {
...
const sessionResults = orderedSessions.filter((s) => s.type !== "race");
  ...
  useEffect(() => {
    const raceName = content.raceName ? ` – ${content.raceName}` : "";
    document.title = `Previous Results${raceName} | KC's Worldwide F1 Update`;
   }, [content.raceName]);
  ...
  return (
    <div className="relative min-h-screen w-full max-w-full overflow-x-hidden text-white">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col gap-3 overflow-x-hidden px-4 pb-6 pt-1 sm:pt-2">
        <SiteHeader />
{showCountdown ? <CountdownBar /> : null}
            ...
            <article className="mt-4 flex min-w-0 flex-col overflow-hidden rounded-3xl border border-white/10 bg-black/30 p-3 backdrop-blur lg:hidden">
              <div>
                ...
                <div className="mt-3 rounded-2xl border border-black/20 bg-gray-50 px-3 py-2 text-xs text-gray-900">
                  {Array.isArray(content.weather) ? (
                    <div className="weather-grid">
                      {content.weather.map((day) => (
...
{gallery?.enabled ? (
  ...
  {sessionResults.map((s) => (
    <React.Fragment key={s.id || s.label}>
      ...
      <MobileSessionRecapCard session={s} recap={recap} />
    </React.Fragment>
  ))}
  ...
  {raceSession ? (
    <React.Fragment key={raceSession.id || raceSession.label || "race"}>
      ...
      <MobileSessionRecapCard session={raceSession} recap={recap} />
    </React.Fragment>
  ) : null}
</div>
</div>
...
{gallery?.enabled ? (
  ...
  <RaceWeekendRecapCard recap={recap} />
</div>
</div>
        </section>


<RaceGalleryCard gallery={gallery} />
<div className="mx-auto mt-8 max-w-4xl rounded-3xl border border-white/10 bg-white/[0.04] p-5 text-center text-sm text-slate-300 shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
  Full Race Centre-style archives begin with the Barcelona Grand Prix.
  Earlier 2026 results were published in the original results format before this expanded archive layout was introduced.
  Sorry for the inconvenience, and stay tuned for more updates and features as the season progresses.
</div>
        <div className="mt-2">
          <AdBar />
        </div>
