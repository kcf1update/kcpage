      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 pt-1 sm:pt-4 pb-8 lg:px-8">
        ...
        <main className="mt-6 grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {slots.map((slot, idx) => {
            ...
            // ✅ NEW: optional external watch button (manual flag per slot)
            const showWatchButton = !!slot?.forceExternal;
            const videoIdForWatch = extractYouTubeId(slot?.videoId || slot?.youtubeInput || "");
            const watchUrl = videoIdForWatch ? `https://www.youtube.com/watch?v=${videoIdForWatch}` : "";
...
{showWatchButton ? (
  <p className="text-xs text-blue-300">
    Can’t play here.. Watch it on YouTube below.
  </p>
) : null}

                  {/* ✅ NEW: Watch on YouTube button (only when forceExternal: true) */}
                  {showWatchButton && watchUrl ? (
                    <a
                      href={watchUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="mt-1 inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600/90 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-600 transition shadow-[0_0_15px_rgba(59,130,246,0.6)]"
                    >
                      ▶ Watch on YouTube ↗
                    </a>
                  ) : null}
