export const newsSlots = [
  {
  slotId: 1,
  sourceLabel: "The Race",
  title:
    "Russell Upstages Antonelli to Take Dutch GP Sprint Pole",
  summary:
    "George Russell claimed pole position for the Dutch Grand Prix sprint at Zandvoort. The Mercedes driver secured his third sprint pole of the season in an extremely close session, with less than a tenth of a second separating the top four.",
  kcsQuickShift: "Should be a good sprint race. Lets see how the start goes",
  url: "https://www.the-race.com/formula-1/dutch-gp-f1-sprint-qualifying-2026-russell-pole/",
  imagePath: "/img/news/xpb/Mercedes/xpb-russellpolezandvoort.jpg",
  photoCredit: "XPB IMAGES",
  dateLabel: "August 21, 2026",
},

{
  slotId: 2,
  sourceLabel: "RacingNews365.nl",
  title:
    "Werk aan de winkel voor Verstappen en Red Bull in Zandvoort | Verstappen and Red Bull Have Work to Do at Zandvoort",
  summary:
    "Max Verstappen kwam tijdens de enige vrije training voor de Grand Prix van Nederland niet verder dan de elfde plaats. De Nederlander klaagde opnieuw over problemen bij het terugschakelen, die voor onbalans in de Red Bull zorgden. | Max Verstappen finished only 11th in the sole practice session for the Dutch Grand Prix. He again complained about downshift problems that were causing instability in the Red Bull.",
  kcsQuickShift: "GP and Max are the best team at finding what they need between practices to go fast when they are back on the track. | GP en Max vormen het beste team als het erop aankomt om tussen de sessies uit te zoeken wat ze nodig hebben om snel te zijn zodra ze weer de baan op gaan. ",
  url: "https://racingnews365.nl/werk-aan-de-winkel-voor-verstappen-en-red-bull-in-zandvoort",
  imagePath: "/img/news/xpb/RedBull/xpb-maxzandvoort.jpg",
  photoCredit: "XPB IMAGES",
  dateLabel: "August 21, 2026",
},
{
  slotId: 3,
  sourceLabel: "RacingNews365",
  title: "Red Bull Provides Isack Hadjar Health Update After Wrist Injury",
  summary:
    "Red Bull says Isack Hadjar’s wrist injury is minor, but the team will not risk putting him back in the car too soon. Liam Lawson replaces him at Red Bull for the Dutch Grand Prix, while Yuki Tsunoda returns with Racing Bulls.",
  kcsQuickShift: "Doesn't sound too serious; hopefully he is not out too long",
  url: "https://racingnews365.com/red-bull-provide-isack-hadjar-health-update-after-wrist-injury",
  imagePath: "/img/news/xpb/RedBull/xpb-isackspa.jpg",
  photoCredit: "XPB IMAGES",
  dateLabel: "August 21, 2026",
},

{
  slotId: 4,
  sourceLabel: "FormulaPassion.it",
  title:
    "Olanda, Ferrari: ecco gli aggiornamenti per irrobustire la SF-26 | Dutch GP: Ferrari Reveals Updates Designed to Strengthen the SF-26",
  summary:
    "La Ferrari ha introdotto a Zandvoort modifiche al fondo, al diffusore e alla beam wing della SF-26. Gli aggiornamenti puntano ad aumentare il carico aerodinamico e a rendere più stabile il flusso d’aria verso il retrotreno. | Ferrari has introduced changes to the SF-26 floor, diffuser and beam wing at Zandvoort. The updates are intended to increase aerodynamic load and improve airflow stability toward the rear of the car.",
  kcsQuickShift: "Sounds like some big changes to some cars this weekend, it'll be interesting to see who benefits the most. | Sembra che questo fine settimana ci siano grandi novità su diverse monoposto. Sarà interessante vedere chi ne trarrà il maggior beneficio. ",
  url: "https://www.formulapassion.it/f1/f1-analisi-tecnica/olanda-ferrari-ecco-aggiornamenti-per-irrobustire-sf-26-foto",
  imagePath: "/img/news/xpb/ferrari/xpb-charleshungary.jpg",
  photoCredit: "XPB IMAGES",
  dateLabel: "August 21, 2026",
},

{
  slotId: 5,
  sourceLabel: "RacingNews365",
  title:
    "Charles Leclerc Warns Against Trying to Follow Max Verstappen’s Direction",
  summary:
    "Charles Leclerc says it would be a mistake to copy Max Verstappen’s route to success with Red Bull. The Ferrari driver wants to pursue a Formula 1 championship in his own way.",
  kcsQuickShift: "To be honest I am not sure what Leclerc is trying to say here?",
  url: "https://racingnews365.com/charles-leclerc-delivers-warning-over-max-verstappen-direction-mistake",
  imagePath: "/img/news/xpb/ferrari/news-xpbcharles.jpg",
  photoCredit: "XPB IMAGES",
  dateLabel: "August 21, 2026",
},

{
  slotId: 6,
  sourceLabel: "PlanetF1.com",
  title:
    "Ferrari Update Plan Continues as Alpine Brings Major Zandvoort Package",
  summary:
    "Alpine has introduced the largest Dutch Grand Prix upgrade package with eight changes to the A526. Ferrari brought five updates focused mainly on the floor and diffuser, while McLaren, Mercedes, Red Bull and Aston Martin also made changes.",
  kcsQuickShift: "Lets hope it gives them a bit more of the mojo they had in the beginning of the year ",
  url: "https://www.planetf1.com/news/ferrari-update-plan-continues-alpine-zandvoort-upgrades",
  imagePath: "/img/news/Alpine/news-alpinejapan.jpg",
  photoCredit: "Courtesy of Alpine F1",
  dateLabel: "August 21, 2026",
},

{
  slotId: 7,
  sourceLabel: "Formula1.com",
  title:
    "Alonso in No Hurry to Make a Decision About His Aston Martin Future",
  summary:
    "Fernando Alonso says deciding whether he will race in Formula 1 in 2027 is not his biggest priority. He expects to speak with Aston Martin about his plans but remains focused on helping the team improve.",
  kcsQuickShift: "Take your time and see what the car is like after the upgrades with the chassis and engine. ",
url: "https://www.formula1.com/en/latest/article/its-not-my-biggest-priority-alonso-in-no-hurry-to-make-call-on-his-aston-martin-future.35uUOTMKs5twJIc1aJ6HJU",
  imagePath: "/img/news/shut/Aston/news-shutfernando.jpg",
  photoCredit: "Shutterstock",
  dateLabel: "August 21, 2026",
},
{
  slotId: 8,
  sourceLabel: "RacingNews365.nl",
  title:
    "Red Bull onthult ‘simpele’ waarheid over Verstappen | Red Bull Reveals the ‘Simple’ Truth About Verstappen",
  summary:
    "Laurent Mekies zegt dat Verstappen tijdens alle gesprekken duidelijk heeft gemaakt dat hij bij Red Bull wilde blijven. Ondanks frustraties over de moeilijke seizoensstart bleef de gezamenlijke wil om te winnen volgens Mekies de basis van de verlenging. | Laurent Mekies says Verstappen made it clear throughout the negotiations that he wanted to remain with Red Bull. Despite frustrations over the team’s difficult start to the season, their shared determination to win remained the foundation of the new agreement.",
  kcsQuickShift: "It ends a season long story that we do not have to talk about any longer",
  url: "https://racingnews365.nl/red-bull-onthult-simpele-waarheid-over-verstappen",
  imagePath: "/img/news/xpb/RedBull/xpbverstappen.jpg",
  photoCredit: "XPB IMAGES",
  dateLabel: "August 21, 2026",
},

{
  slotId: 9,
  sourceLabel: "Motorsport.com",
  title:
    "Jolyon Palmer Casts Doubt on Esteban Ocon’s Haas Formula 1 Future",
  summary:
    "Jolyon Palmer believes Esteban Ocon is in serious trouble at Haas following a difficult first half of the season and strong performances from Oliver Bearman. Ocon’s contract expires at the end of 2026 as Haas considers its options for next year.",
  kcsQuickShift: "His star has fallen very quickly ever since the crash with Gasly at Monaco after which he almost lost his ride on the spot.",
  url: "https://www.motorsport.com/f1/news/jolyon-palmer-casts-doubt-on-esteban-ocons-haas-f1-future-really-in-trouble/10847857/",
  imagePath: "/img/news/xpb/Haas/news-xpbestoban.jpg",
  photoCredit: "XPB IMAGES",
  dateLabel: "August 21, 2026",
},

{
  slotId: 10,
  sourceLabel: "The Race",
  title:
    "McLaren’s Crinkled Rear Wing Among Dutch Grand Prix Upgrades",
  summary:
    "McLaren has arrived at Zandvoort with an unusually shaped rear wing and revised rear brake ducts. The changes complete the development package introduced in Hungary and are intended to improve the MCL40’s aerodynamic performance.",
  kcsQuickShift: "Lando is talking stong. Lets see what they brought",
  url: "https://www.the-race.com/formula-1/crinkled-mclaren-rear-wing-among-dutch-gp-f1-upgrades/",
  imagePath: "/img/news/xpb/Mclaren/xpb-landohungary2.jpg",
  photoCredit: "XPB IMAGES",
  dateLabel: "August 21, 2026",
},
];